...
- Cash: pulls per-currency CashBalance from IB accountValues(), converts to USD using yfinance FX,
        and includes as a single combined slice "CASH" (across all currencies).
...
def get_cash_balances_from_ib(ib: IB) -> Dict[str, float]:
    """
    Returns cash balances per currency from IB accountValues().
...
def main():
    ...
    try:
        ...
        # Collect currencies for FX (portfolio + cash)
        currencies = set()
        for p in portfolio:
        ...
        # Aggregate exposures by underlying symbol (stock symbol)
        net_usd_by_underlying: Dict[Tuple[str, str], float] = {}  # (symbol, currency) -> USD value

        # ---- CASH: combined into one slice (CASH, USD) ----
        skipped_cash_fx = 0
        cash_usd_total = 0.0
        for ccy, amt in cash_by_ccy.items():
            ...
            if ccy not in fx_map:
                skipped_cash_fx += 1
                continue
            cash_usd_total += amt * fx_map[ccy]

        if abs(cash_usd_total) > 1e-9:
            net_usd_by_underlying[(CASH_LABEL, "USD")] = cash_usd_total
        ...
        model_box = (
            ...
            f"Cash: IB CashBalance (combined into '{CASH_LABEL}')\n"
